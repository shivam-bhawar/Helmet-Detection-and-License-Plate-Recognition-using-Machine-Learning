...
class PlateFinder:
    ...
    def preprocess(self, input_img): #1
        imgBlurred = cv2.GaussianBlur(input_img, (7, 7), 0)
        gray = cv2.cvtColor(imgBlurred, cv2.COLOR_BGR2GRAY)  
        sobelx = cv2.Sobel(gray, cv2.CV_8U, 1, 0, ksize=3)  
        ret2, threshold_img = cv2.threshold(sobelx, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU) #Otsu's Threshhold

        element = self.element_structure
        morph_n_thresholded_img = threshold_img.copy()
        cv2.morphologyEx(src=threshold_img, op=cv2.MORPH_CLOSE, kernel=element, dst=morph_n_thresholded_img) #Closing Morphological Transaction
        return morph_n_thresholded_img

    def extract_contours(self, after_preprocess): #2
        _, contours, _ = cv2.findContours(after_preprocess, mode=cv2.RETR_EXTERNAL,
                                                    method=cv2.CHAIN_APPROX_NONE)
        return contours

    def clean_plate(self, plate): #3
        ...
        if ret == True:
            ...
            if possible_plates is not None:
                for i, p in enumerate(possible_plates):
                   ...
                   # messagebox.showinfo('RTO Rules Regarding Helmet', 'You are not wearing helmet so you have a 500 Rs. Fine')
                    cv2.imshow('plate', p)
                    if cv2.waitKey(25) & 0xFF == ord('q'):
                        break
